...
void Building::ServerCreateBuildingActor(UObject* Context, FFrame& Stack)
{
	...
	ABuildingSMActor* Building = Utils::FinishSpawnActor<ABuildingSMActor>(Utils::SpawnActorUnfinished<ABuildingSMActor>(BuildingClass, CreateBuildingData.BuildLoc, CreateBuildingData.BuildRot, PlayerController), CreateBuildingData.BuildLoc, CreateBuildingData.BuildRot);
	if (!Building)
		return;
	...
	if (auto ControllerAthena = PlayerController->Cast<AFortPlayerControllerAthena>())
		ControllerAthena->BuildingsCreated++;
...
void Building::ServerEditBuildingActor(UObject* Context, FFrame& Stack)
{
	...
	static auto ReplaceBuildingActor = (ABuildingSMActor * (*)(ABuildingSMActor*, unsigned int, UObject*, unsigned int, int, bool, AFortPlayerController*))(Sarah::Offsets::ImageBase + 0x9CB8FD8);
...
void Building::ServerCreateBuildingAndSpawnDeco(UObject* Context, FFrame& Stack) {
	...
	Building = Utils::FinishSpawnActor<ABuildingSMActor>(Utils::SpawnActorUnfinished<ABuildingSMActor>(Build, BuildingLocation, BuildingRotation, PlayerController), BuildingLocation, BuildingRotation);
	Building->bPlayerPlaced = true;
	Building->InitializeKismetSpawnedBuildingActor(Building, PlayerController, true, nullptr);
	Building->TeamIndex = ((AFortPlayerStateAthena*)PlayerController->PlayerState)->TeamIndex;
	Building->Team = EFortTeam(Building->TeamIndex);
...
void Building::Hook() {
	...
	Utils::Hook(Sarah::Offsets::ImageBase + 0x9C2879C, OnDamageServer, OnDamageServerOG);
