...
FVector GetActorLocation(AActor* Actor)
{
	...
	return RootComponent ? *(FVector*)(__int64(RootComponent) + 0x260) : FVector();
}
...
void Replication::BuildPriorityLists(UNetDriver* Driver, float DeltaTime)
{
	...
	if (ActiveNetworkObjectsNum/* != ActiveNetworkObjects.Num()*/ == 0)
	{
		...
		Utils::Hook(Sarah::Offsets::ImageBase + 0x7FBF80C, ForceNetUpdate);
...
UNetConnection* Replication::IsActorOwnedByAndRelevantToConnection(const AActor* Actor, TArray<FNetViewer>& ConnViewers, bool& bOutHasNullViewTarget)
{
	...
	for (auto& Viewer : ConnViewers)
	{
		...
		if (Conn->ViewTarget == nullptr) [[unlikely]]
		{
		...
		if (ActorOwner == Conn->PlayerController ||
			...
			(Conn->ViewTarget && IsRelevancyOwnerFor(Conn->ViewTarget, Actor, ActorOwner, Conn->OwningActor))) [[unlikely]]
		{
			return Conn;
		}
	}
...
{
	if (Actor->NetDormancy <= ENetDormancy::DORM_Awake || !Channel || Channel->bPendingDormancy || Channel->Dormant) [[unlikely]]
	{
		// Either shouldn't go dormant, or is already dormant
		return false;
	}

	if (Actor->NetDormancy == ENetDormancy::DORM_DormantPartial) [[unlikely]]
	{
		//Log(L"dorm partial??? %s", ActorInfo->GetWName().c_str());
		for (int32 viewerIdx = 0; viewerIdx < ConnectionViewers.Num(); viewerIdx++)
		{
			//if (!GetNetDormancy(ActorInfo, ConnectionViewers[viewerIdx].ViewLocation, ConnectionViewers[viewerIdx].ViewDir, ConnectionViewers[viewerIdx].InViewer, ConnectionViewers[viewerIdx].ViewTarget, Channel, Time, bLowNetBandwidth))
			{
				return false;
			}
		}
...
{
	auto IsLevelInitializedForActor = (bool (*&)(const UNetDriver*, const AActor*, UNetConnection*)) NetDriver->VTable[0x87];
	return IsLevelInitializedForActor(NetDriver, InActor, InConnection);
}

void Replication::SendClientAdjustment(APlayerController* PlayerController)
{
	static auto SendClientAdjustment = (void (*)(APlayerController*)) (Sarah::Offsets::ImageBase + 0x80B4948);
	...
	for (auto& PriorityActor : PriorityActors)
	{
		...
		if (!Channel || Channel->Actor) [[likely]]
		{
			if (!Channel) [[unlikely]]
			{
				...
				static auto ActorFName = (FName*)(Sarah::Offsets::ImageBase + 0x108145d8);
				Channel = CreateChannelByName(Conn, ActorFName, EChannelCreateFlags::OpenedLocally, -1);
...
{
	Utils::Hook(Sarah::Offsets::ImageBase + 0x7FB9574, AddDestructionInfo, AddDestructionInfoOG);
	Utils::Hook(Sarah::Offsets::ImageBase + 0x7E5F61C, RemoveDestructionInfo, RemoveDestructionInfoOG);
	//Utils::Hook(ReplicationOffsets::RemoveNetworkActor, RemoveNetworkActor, RemoveNetworkActorOG);
	//Utils::Hook(Sarah::Offsets::ImageBase + 0x1179BAC, AddNetworkActor, AddNetworkActorOG);
}